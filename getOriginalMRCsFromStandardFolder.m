...
if isfield(configuration, "raw_files_folder") && flatten == true
    mrc_path = configuration.processing_path + string(filesep) + configuration.output_folder + string(filesep)...
        + configuration.raw_files_folder + string(filesep) + "**" + string(filesep) + "*.mrc";
    original_mrcs = dir(mrc_path);
elseif isfield(configuration, "raw_files_folder") && flatten == false
    mrc_path = configuration.processing_path + string(filesep) + configuration.output_folder + string(filesep) + configuration.raw_files_folder;
    ...
    for i = 1:length(original_mrc_folders)
        original_mrcs{i} = dir(mrc_path + string(filesep) + original_mrc_folders(i).name + string(filesep) + "*.mrc");
    end
