...
classdef crYOLO < Module
    methods
        ...
        function obj = config(obj, step_params)
            ...
            use_params_cell = {'filter', 'input_size'}; 
            ...
            if step_params.train_mode == true
                ...
                if ~isfolder(step_params.train_tomograms_folder) || isempty(step_params.train_tomograms_folder)
                    if step_params.train_tomograms_path == ""
                        obj.LinkTomogramsToRequestedDirectory(step_params.tomograms_binning, step_params.train_tomograms_folder);
                    else
                        obj.LinkTomogramsToRequestedDirectory(step_params.tomograms_binning, step_params.train_tomograms_folder, step_params.train_tomograms_path);
                    end
        ...
        function obj = predict(obj, step_params)
            ...
            if ~isfolder(step_params.test_tomograms_folder) || isempty(step_params.test_tomograms_folder)
                if step_params.test_tomograms_path == ""
                    obj.LinkTomogramsToRequestedDirectory(step_params.tomograms_binning, step_params.test_tomograms_folder);
                else
                    obj.LinkTomogramsToRequestedDirectory(step_params.tomograms_binning, step_params.test_tomograms_folder, step_params.test_tomograms_path);
                end
        ...
        function LinkTomogramsToRequestedDirectory(obj, tomograms_binning, tomograms_folder_dest, tomograms_folder_src)
            
            if nargin < 3
                tomograms = getCtfCorrectedBinnedTomogramsFromStandardFolder(obj.configuration, true);
            else
                tomograms = dir(tomograms_folder_src + string(filesep) + "*.rec");
                if isempty(tomograms)
                   tomograms = dir(tomograms_folder_src + string(filesep) + "*.mrc"); 
                end
            end
            ...
            tomograms_path = obj.output_path + string(filesep) + tomograms_folder_dest;
