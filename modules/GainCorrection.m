classdef GainCorrection < Module
    methods
        ...
        function obj = process(obj)            
            %             % TODO: account for case where input prefix is missing
            %             data_path = obj.configuration.data_path + string(filesep) + obj.configuration.tomogram_input_prefix + "*.mrc";
            %
            %             data_dir = dir(data_path);
            %             tif_flag = false;
            %             if isempty(data_dir)
            %                 data_path = obj.configuration.data_path + string(filesep) + obj.configuration.tomogram_input_prefix + "*.tif";
            %                 data_dir = dir(data_path);
            %                 tif_flag = true;
            %             end
            %
            %             if tif_flag == false
            %                 tmp_image_size = size(dread(char(data_dir(1).folder + string(filesep) + data_dir(1).name)));
            %             else
            %                 info = imfinfo(char(data_dir(1).folder + string(filesep) + data_dir(1).name));
            ...
            %[data_dir, tif_flag] = getOriginalMRCsorTIFs(obj.configuration);
...
%             data_dir = strcat({obj.configuration.original_files(:).folder}, string(string(filesep)), {obj.configuration.original_files(:).name});
            ...
            if obj.configuration.parallel_execution == true
                %generatePool(obj.configuration.cpu_fraction, false, obj.configuration.processing_path + filesep + obj.configuration.output_path + "jobs");
                parfor i = 1:length(data_dir)
                    image = [];
%                     file_name = data_dir(i).folder + string(filesep) + data_dir(i).name;
                    ...
                    if tif_flag == false
                        ...
                        %info = imfinfo(char(data_dir(i).folder + string(filesep) + data_dir(i).name));
                        info = imfinfo(char(data_dir(i)));
                        num_images = numel(info);
                        counter = 1;
                        for k = 1:num_images
                            %image(:,:,counter) = double(imread(char(data_dir(i).folder + string(filesep) + data_dir(i).name), k))';
...
%                     disp("INFO:VARIABLE:file_name -> " + file_name);
%                     if tif_flag == false
%                         image = double(dread(char(data_dir(i).folder + string(filesep) + data_dir(i).name)));
%                         % TODO extract to function and add mean value to
%                         % boarder
%                         if size(image,1) ~= tmp_image_size(1) || size(image,2) ~= tmp_image_size(2)
%                             difference = abs(tmp_image_size(1) - size(image,1));
