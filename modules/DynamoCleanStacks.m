classdef DynamoCleanStacks < Module
    methods
        ...
        function obj = setUp(obj)
            ...
            %             	createStandardFolder(obj.configuration, "tilt_stacks_folder", true);
            %             end
        end
        
        function obj = process(obj)
            %             if obj.configuration.set_up.j == 1
            %                 createStandardFolder(obj.configuration, "tilt_stacks_folder", true);
            %             else
            %                 createStandardFolder(obj.configuration, "tilt_stacks_folder", false);
            %             end
            ...
            %             excluded_tilt_stacks_and_tilts = struct;
            
            
            stack_file = dir_list;
            % NOTE: 3 for third entry because of "." and ".." ???sure???
            ...
            %             destination_path = obj.configuration.processing_path + string(filesep) + obj.configuration.output_folder + string(filesep) + obj.configuration.tilt_stacks_folder + string(filesep) + name;
            %             destination = destination_path + string(filesep) + name + ".st";
            ...
            dynamo_success_file = dir(obj.configuration.processing_path...
                ...
                + "SUCCESS_*");
            dynamo_success_file_splitted = strsplit(dynamo_success_file.name, "_");
            ...
            if ~isempty(dynamo_tilt_series_alignment_folder)
                % TODO: test if you can hand over the index in json
                % partials and don't use files
%                 index_file = dir(obj.configuration.processing_path...
%                     + string(filesep) + obj.configuration.output_folder + string(filesep)...
%                     + "*DynamoTiltSeriesAlignment*" + string(filesep) + name + string(filesep)...
%                     + "INDEX");
%                 index_fid = fopen(index_file(1).folder + string(filesep) + index_file(1).name);
%                 % TODO: decide how to handle
%                 index = fscanf(index_fid, "%s");
%                 index = str2num(index);
%                 % TODO: dirty
%                 if isfield(obj.configuration.tomograms.(field_names{obj.configuration.set_up.j}), "tilt_series_alignment_index")
%                     index = obj.configuration.tomograms.(field_names{obj.configuration.set_up.j}).tilt_series_alignment_index;
%                 end
                index = 1;
                ...
                %                 excluded_tilt_stacks_and_tilts.(stack_file_name_splitted{1}) = tilts_to_be_removed;
                ...
                if length(motion_corrected_files) > length(tilts_to_keep)
                    ...
                    %                     tilt_index_angle_mapping = obj.dynamic_configuration.tilt_index_angle_mapping;
                    ...
                    %[success, message, message_id] = rmdir(obj.configuration.tomograms.(field_names{obj.configuration.set_up.j}).tilt_stack_path, "s");
                ...
                if obj.configuration.propagate_failed_stacks == true
                    ...
                    %excluded_tilt_stacks_and_tilts.(stack_file_name_splitted{1}) = [];
