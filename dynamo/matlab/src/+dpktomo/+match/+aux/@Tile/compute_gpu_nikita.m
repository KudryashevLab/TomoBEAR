...
%
chunkNormalized = gpuArray(dynamo_normalize_roi(chunk));
...
tdrotMatrix  = 0*ones(Nx,Ny,Nz,'int16', "gpuArray"); %, "gpuArray"
tiltMatrix   = 0*ones(Nx,Ny,Nz,'int16', "gpuArray"); %, "gpuArray"
narotMatrix  = 0*ones(Nx,Ny,Nz,'int16', "gpuArray"); %, "gpuArray"
...
clear gpu_x_coordinates gpu_y_coordinates gpu_z_coordinates gpu_h_coordinates;
...
mem_difference_per_iteration = 0;
for i=1:NAngles
    iteration_begin = tic;
    iteration_file_id = fopen(string(pwd) + string(string(filesep)) + "ITERATION", "w+");

    fprintf(iteration_file_id, "%d/%d", i, NAngles);
    ...
    if std(rotatedTemplate(rotatedMask > 0)) < eps
        disp('-------------------------------------------------');
...
%     
%     if i == 1
%         output = executeCommand("nvidia-smi");
%         [mem_mat, mem_tok, mem_ext] = regexp(output, "\|[\s]+" + (gpu_device.Index  - 1) + "[\s]+[0-9]+[\s]+C[\s]+...[\w\/]+[\s]+([0-9]+)MiB \|", "match", "tokens", "tokenExtents");
%         mem_first = str2double(mem_tok{1});
%     end
...
%     output = executeCommand("nvidia-smi");
%     [mem_mat, mem_tok, mem_ext] = regexp(output, "\|[\s]+" + (gpu_device.Index - 1) + "[\s]+[0-9]+[\s]+C[\s]+...[\w\/]+[\s]+([0-9]+)MiB \|", "match", "tokens", "tokenExtents");
%     mem_second = str2double(mem_tok{1});
%     if i == 1
%         mem_difference_per_iteration = mem_second - mem_first;
%     end
    
%     j = j + 1;
%     if mod(i, 15) == 0 || i == NAngles || (mem_second * 1024 * 1024) + (mem_difference_per_iteration * 1.1 * 1024 * 1024) > gpu_device.AvailableMemory
%         for j=1:length(gpu_ccForBlock)
%             ccForBlock{i-j + 1} = gather(gpu_ccForBlock{j});
%         end
%         j = 1;
%         clear gpu_ccForBlock;
%     end
