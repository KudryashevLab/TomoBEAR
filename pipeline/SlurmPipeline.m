    ...
    methods
        ...
        function execute(obj, tomogram_begin, tomogram_end, pipeline_ending_step)
            ...
            while file_count_changed == true
                ...
                if length(configuration.general.nodes) == 1
                    [job_ids, first_step_to_execute] = obj.queueJobs(configuration, previous_job_ids, first_step_to_execute, 1, dynamic_configuration.tomograms_count, -1, "TomoBEAR", configuration.general.nodes(1), -1);
                else
                        ...
                        elseif fileExists(failure_file_path) && merged_configuration.ignore_success_files == false
                            ...
                            elseif isfield(merged_configuration, "execution_method")...
                                ...
                                for j = 0:ceil(dynamic_configuration.tomograms_count/configuration.general.jobs_per_node) - 1
                                    ...
                                    if mod(j, length(configuration.general.nodes)) == length(configuration.general.nodes) - 1 && j < ceil(dynamic_configuration.tomograms_count/configuration.general.jobs_per_node) - 1 && j > 0  && configuration.general.slurm_exclusive ~= true
