...
if isfield(configuration, "ctf_corrected_tomograms_folder") && flatten == true
    tomograms_path = configuration.processing_path + string(filesep) + configuration.output_folder + string(filesep)...
        + configuration.ctf_corrected_tomograms_folder + string(filesep) + "**" + string(filesep) + "*.rec";
    
    tomograms = dir(tomograms_path);
    if length(tomograms) == 0
        tomograms_path = configuration.processing_path + string(filesep) + configuration.output_folder + string(filesep)...
        + configuration.ctf_corrected_tomograms_folder + string(filesep) + "**" + string(filesep) + "*.mrc";
    
        tomograms = dir(tomograms_path);
    end
elseif isfield(configuration, "ctf_corrected_tomograms_folder") && flatten == false
    tomograms_path = configuration.processing_path + string(filesep) + configuration.output_folder + string(filesep) + configuration.ctf_corrected_tomograms_folder;
