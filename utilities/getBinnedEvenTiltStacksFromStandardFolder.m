...
function tilt_stacks = getBinnedEvenTiltStacksFromStandardFolder(configuration, flatten)
if nargin == 1
    flatten = false;
end

if isfield(configuration, "binned_even_tilt_stacks_folder") && flatten == true
    tilt_stack_path = configuration.processing_path + string(filesep)...
        + configuration.output_folder + string(filesep)...
        + configuration.binned_even_tilt_stacks_folder...
        + string(filesep) + "**" + string(filesep) + "*.st";
    tilt_stacks = dir(tilt_stack_path);
elseif isfield(configuration, "binned_even_tilt_stacks_folder") && flatten == false
    tilt_stack_path = configuration.processing_path + string(filesep)...
        + configuration.output_folder + string(filesep)...
        + configuration.binned_even_tilt_stacks_folder;
