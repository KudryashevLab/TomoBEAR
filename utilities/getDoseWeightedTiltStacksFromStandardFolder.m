...
function tilt_stacks = getDoseWeightedTiltStacksFromStandardFolder(configuration, flatten)
if nargin == 1
    flatten = false;
end

if isfield(configuration, "dose_weighted_tilt_stacks_folder") && flatten == true
    tilt_stack_path = configuration.processing_path + string(filesep)...
        + configuration.output_folder + string(filesep)...
        + configuration.dose_weighted_tilt_stacks_folder...
        + string(filesep) + "**" + string(filesep) + "*.st";
    tilt_stacks = dir(tilt_stack_path);
elseif isfield(configuration, "dose_weighted_tilt_stacks_folder") && flatten == false
    tilt_stack_path = configuration.processing_path + string(filesep)...
        + configuration.output_folder + string(filesep)...
        + configuration.dose_weighted_tilt_stacks_folder;
...
if isempty(tilt_stacks)
    disp("INFO: No dose_weighted tilt stacks found at standard location " + tilt_stack_path);
end
end
